...
const UpdateCompanyForm = ({ company, handleSubmit, background, setBackground, error, setFile, imgUrl }) => {
  return (
    <Formik enableReinitialize={true} initialValues={company} validationSchema={validateCompany} onSubmit={handleSubmit}>
      {({ isSubmitting }) => (
        <Form className={styles.update_form}>
          <label className={styles.change_image} htmlFor="newFileUrl">
            ...
            <br />
            <img loading="lazy" src={imgUrl || company.fileUrl} width={"50px"} style={{ padding: "5px" }} alt="Company logo" />
            <input
              onChange={setFile}
              type={"file"}
              name="newFileUrl"
            ...
            />
            <img loading="lazy" className={styles.add_image} src="/assets/camera.png" alt="Add company logo" />
          </label>
          ...
          <CustomEditor background={background || company.background} setBackground={setBackground} />
